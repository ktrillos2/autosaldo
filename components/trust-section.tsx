...
export function TrustSection({ content }: { content?: TrustContent }) {
  const overline = content?.overline || "Proceso transparente"
  const title = content?.title || "Así trabajamos en Autosaldo"
  const description = content?.description || "Descubre nuestro proceso claro y seguro para vender tu vehículo."
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          ...
          >
            ...
            <span className="text-[#d30826] font-medium tracking-wider uppercase">{overline}</span>
          </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {steps.map((step, index) => {
            const IconComponent = iconsMap[step.iconName] || Phone
            return (
              <motion.div
                key={index}
